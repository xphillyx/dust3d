...
void AxisAlignedBoudingBoxTree::splitNode(Node *node)
{
    ...
    const Vertex &lower = splitBox.lowerBound();
    const Vertex &upper = splitBox.upperBound();
    std::vector<std::pair<size_t, float>> spans(3);
    for (size_t i = 0; i < 3; ++i)
        spans[i] = {i, upper.xyz[i] - lower.xyz[i]};
    ...
    auto splitPoint = node->center.xyz[longestAxis];
    node->left = new Node;
    node->right = new Node;
