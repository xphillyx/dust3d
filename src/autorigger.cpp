        ...
        const std::set<MeshSplitterTriangle> &markTriangles)
{
    ...
    if (isCutOffSplitter(mark.boneMark)) {
        if (!mark.split(m_inputTriangles)) {
            m_messages.push_back(std::make_pair(QtCriticalMsg,
                tr("Mark \"%1\" couldn't cut off the mesh").arg(SkeletonBoneMarkToString(mark.boneMark))));
            return false;
        }
    }
...
bool AutoRigger::validate()
{
    ...
    if (neckIndicies == m_marksMap.end()) {
        foundError = true;
        m_messages.push_back(std::make_pair(QtCriticalMsg,
            tr("Couldn't find neck mark")));
    }
    if (leftShoulderIndicies == m_marksMap.end()) {
        foundError = true;
        m_messages.push_back(std::make_pair(QtCriticalMsg,
            tr("Couldn't find left shoulder mark")));
    }
    if (leftElbowIndicies == m_marksMap.end()) {
        foundError = true;
        m_messages.push_back(std::make_pair(QtCriticalMsg,
            tr("Couldn't find left elbow mark")));
    }
    if (leftWristIndicies == m_marksMap.end()) {
        foundError = true;
        m_messages.push_back(std::make_pair(QtCriticalMsg,
            tr("Couldn't find left wrist mark")));
    }
    if (rightShoulderIndicies == m_marksMap.end()) {
        foundError = true;
        m_messages.push_back(std::make_pair(QtCriticalMsg,
            tr("Couldn't find right shoulder mark")));
    }
    if (rightElbowIndicies == m_marksMap.end()) {
        foundError = true;
        m_messages.push_back(std::make_pair(QtCriticalMsg,
            tr("Couldn't find right elbow mark")));
    }
    if (rightWristIndicies == m_marksMap.end()) {
        foundError = true;
        m_messages.push_back(std::make_pair(QtCriticalMsg,
            tr("Couldn't find right wrist mark")));
    }
    if (leftHipIndicies == m_marksMap.end()) {
        foundError = true;
        m_messages.push_back(std::make_pair(QtCriticalMsg,
            tr("Couldn't find left hip mark")));
    }
    if (leftKneeIndicies == m_marksMap.end()) {
        foundError = true;
        m_messages.push_back(std::make_pair(QtCriticalMsg,
            tr("Couldn't find left knee mark")));
    }
    if (leftAnkleIndicies == m_marksMap.end()) {
        foundError = true;
        m_messages.push_back(std::make_pair(QtCriticalMsg,
            tr("Couldn't find left ankle mark")));
    }
    if (rightHipIndicies == m_marksMap.end()) {
        foundError = true;
        m_messages.push_back(std::make_pair(QtCriticalMsg,
            tr("Couldn't find right hip mark")));
    }
    if (rightKneeIndicies == m_marksMap.end()) {
        foundError = true;
        m_messages.push_back(std::make_pair(QtCriticalMsg,
            tr("Couldn't find right knee mark")));
    }
    if (rightAnkleIndicies == m_marksMap.end()) {
        foundError = true;
        m_messages.push_back(std::make_pair(QtCriticalMsg,
            tr("Couldn't find right ankle mark")));
    }
    if (foundError)
        return false;
...
bool AutoRigger::rig()
{
    ...
    QVector3D spineBoneStartPosition = bonesOrigin;
    ...
    if (isMainBodyVerticalAligned) {
        ...
        QVector3D minZ = findMinZ(headVertices);
        headBoneStopPosition = QVector3D(headBoneStartPosition.x(),
            minZ.y(),
            minZ.z());
    }
    ...
    {
        QVector3D minZ = findMinZ(leftLegVertices);
    ...
    {
        QVector3D minZ = findMinZ(rightLegVertices);
